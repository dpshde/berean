...
The live workflow is: Jev Choice zoom Book→Chapter→Verse (parallel beam K=3, `path_score = product(edge_p) ** (1 / decisions)`) → one TypeSafe request (`question_kind` yes_no|free_form, plus speculative `supported`, `denied`, per-verse `supports|contradicts|silent`) on the retained leaf verses → `composeVerdict()`, which emits a yes/no only for polar questions and stitches consecutive same-chapter hits into one range passage.
