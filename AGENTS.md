...
The live workflow is: Jev Choice zoom Book→Chapter→Verse (parallel beam K=3, `path_score = product(edge_p) ** (1 / decisions)`) → one TypeSafe request (`supported`, `denied`, per-verse `supports|contradicts|silent`) on the retained leaf verses → `composeVerdict()`, which stitches consecutive same-chapter, same-relation hits into one range passage.
